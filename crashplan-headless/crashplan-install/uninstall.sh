...
echo "============================================================================"
echo "This portion of the uninstall will remove the Code42 ${APP_BASENAME} software and configuration"
...
if [[ $YN == "yes" ]]; then
	...
	# remove all desktop launchers
	LAUCHER_FILE_NAME=$(echo "${APP_BASENAME}.desktop" | awk '{print tolower($0)}')
	if [[ -f "/usr/share/applications/${LAUCHER_FILE_NAME}" ]]; then
		rm -rf "/usr/share/applications/${LAUCHER_FILE_NAME}"
	fi
	if [[ -f "/home/${SRC_USER}/.local/share/applications/${LAUCHER_FILE_NAME}" ]]; then
		rm -rf "/home/${SRC_USER}/.local/share/applications/${LAUCHER_FILE_NAME}"
	fi
	# remove launchers from desktop, whether named with mixed or lower case.
	if [[ -f "/home/${SRC_USER}/Desktop/${LAUCHER_FILE_NAME}" ]]; then
		rm -rf "/home/${SRC_USER}/Desktop/${LAUCHER_FILE_NAME}"
	fi
	...
	echo ""
	echo ""
	echo "If this computer received incoming backup data, it has NOT been removed."
	echo "It is located in ${MANIFESTDIR}."
	echo "If you reinstall Code42 ${APP_BASENAME} and want to resume from where you"
	echo "left off, you will need to set"
	echo "${MANIFESTDIR}"
	echo "as the location where you would like to store your backups."
else
